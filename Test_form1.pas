...
uses
  ...
  Dialogs, StdCtrls, ComCtrls, ExtCtrls, core, ShareMem;
...
procedure TfrmTest1.btnExitClick(Sender: TObject);
begin
  Self.Close;
end;

procedure TfrmTest1.btnNewGameClick(Sender: TObject);
var
  i: byte;
begin
...
procedure TfrmTest1.btnTurnClick(Sender: TObject);
var
  i: byte;
begin
...
//кнопку хода разблокировать только если оба бойца еще живы
  ...
  begin
    if (bots[0].ifdead = true) then
      Self.mBattleLog.Lines.Add('Персонаж ' + bots[0].name + ' убит. Персонаж ' + bots[1].name + ' победил.');
    if (bots[1].ifdead = true) then
      Self.mBattleLog.Lines.Add('Персонаж ' + bots[1].name + ' убит. Персонаж ' + bots[0].name + ' победил.');
    Self.mBattleLog.Lines.Add('Бой завершен.');
  end;
  Self.btnExit.Enabled:= true;
end;

procedure TfrmTest1.FormClose(Sender: TObject; var Action: TCloseAction);
...
procedure TfrmTest1.FormShow(Sender: TObject);
begin
